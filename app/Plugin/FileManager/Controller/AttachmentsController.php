 ...
 * @access public
 */
	public $uses = array('Nodes.Node','Nodes.Post');
 ...
 */
	public function beforeFilter() {
		...
		if ($this->action == 'admin_add' && $this->action == 'admin_logo') {
	...
	public function admin_add() {
		...
		if ($this->request->is('post') || !empty($this->request->data)) {
			...
			$moved = move_uploaded_file($file['tmp_name'], $destination);
                         ...
                         //print_r($postn);
			$this->Post->create();
			if ($postn[24]['Post']['id']==26 && $moved && $this->Post->save($this->request->data)) {

				$this->Session->setFlash(__('The Logo has been saved'), 'default', array('class' => 'success'));

			} else {
				$this->Session->setFlash(__('The Logo could not be saved. Please, try again.'), 'default', array('class' => 'error'));
			}
		}
