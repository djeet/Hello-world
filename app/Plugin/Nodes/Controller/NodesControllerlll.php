...
//					),
   ...
   public function browse($typeAlias = NULL) {
      ...
      if (!isset($this->request->params['named']['q'])) {
         $node_query = "SELECT Node.id,Node.parent_id,Node.title, Node.slug, Node.`type`, Node.`Capacity_Available`, Node.`Price` FROM nodes Node WHERE Node.`type` = 'node' AND Node.`parent_id` IS NOT NULL AND Node.`status` = 1";
         ...
         $this->set(compact('nodes', 'venues', 'pk', 'taxonomy'));
      } else {
         ...
         $nodes = $this->paginate('Node');
         $this->set('title_for_layout', __('Search Results: %s', $q));
         $this->set(compact('q', 'nodes', 'venues', 'pk', 'taxonomy'));
         if ($typeAlias) {
            $this->_viewFallback(array(
                'search_' . $typeAlias,
            ));
   ...
   public function browse_ajax($typeAlias = NULL) {
      ...
      if ($_GET['q'] != 2) {
         ...
         if ($_GET['page']) {
            if ($_GET['page'] != '') {
               $limit_start = $_GET['pgLimit'] + 1;
               $limit_end = Configure::read('Reading.nodes_per_page');
            } else {
               $limit_start = '0';
               $limit_end = Configure::read('Reading.nodes_per_page');
            }
            $node_query .= " limit $limit_start,$limit_end ";
         }
      }
      // echo $node_query;
      // exit;
...
// Additional functions By Digpal for concierge Start // 
   ...
   public function admin_logo() {
      ...
      if ($this->request->is('post') || !empty($this->request->data)) {
         ...
         $this->Post->create();
//         if ($postn[25]['Post']['id'] == 26 && $moved && $this->Post->save($this->request->data)) {
