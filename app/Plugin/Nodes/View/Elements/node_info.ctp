...
<?php
	...
	if ($type['Type']['format_show_date']) {
		echo ' ' . __('on') . ' ';
		echo $this->Html->tag('span', $this->Time->format(Configure::read('Reading.date_time_format'), $this->Nodes->field('updated'), null, Configure::read('Site.timezone')), array('class' => 'date'));
	}
?>
</div>