 ...
 * @return string
 */
	public function field($key = '', $value = null, $value_x = null, $id = null, $options = array()) {
		//exit;
        $_options = array(
			'key'   => array(
				'label'   => __('key'),
...
//                    'multiple'=>'multiple'
		...
		$fields .= $this->Form->input('Meta.' . $uuid . '.value', $options['value']);
          $fields .= $this->Form->input('Meta.' . $uuid . '.value_x', $options['value_x']);
		
          $this->Form->unlockField('Meta.' . $uuid . '.key');
		$this->Form->unlockField('Meta.' . $uuid . '.value');
          $this->Form->unlockField('Meta.' . $uuid . '.value_x');
		...
		$actions = $this->Html->link(__('Remove'),$deleteUrl,array('class' => 'remove-meta', 'rel' => $id)
		); 
